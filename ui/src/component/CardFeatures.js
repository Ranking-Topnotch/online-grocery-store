...
const CardFeatures = ({name, image, price, category, loading, id}) => {
  ...
  return (
    <div className='w-full py-5 min-w-[200px] max-w-[200px] bg-white shadow-lg hover:drop-shadow-lg pt-5 px-4 cursor-pointer flex flex-col'>
        {
            image ? <>
            <Link to={`/menu/${id}`} onClick={()=>window.scrollTo({top: "0", behaviour: "smooth"})}>
                 <div className='h-14 flex flex-col justify-center items-center'>
                    <img src={image} className='h-full' alt-='cardfeatures' />
                </div>
